...
export default function GameCanvas({
  ...
  useEffect(() => {
    const keyHandler = (e) => {
      console.log(
        `Key pressed: ${e.key}, gameStarted: ${gameStarted}, gameOver: ${gameOver}, isPaused: ${isPaused}`,
      );
  ...
  const createFood = () => {
    ...
    do {
      ...
      overlapsSnake = snakeRef.current.some(
  ...
  const createBomb = () => {
    ...
    do {
      ...
      overlapsBomb = bombsRef.current.some(
  ...
  const updateGame = () => {
    ...
    if (
      ...
      bombsRef.current.length < MAX_BOMBS
    ) {
      createBomb();
    }
  };
  ...
  const drawGame = (context) => {
    ...
    snakeRef.current.forEach((segment, index) => {
      ...
      if (index === 0) {
        ...
        const colorIndex =
          (index + Math.floor(Date.now() / timeMultiplier)) %
        ...
        if (isTimeWarpActive && index < 5) {
          // Only for first few segments for performance
    ...
    bombsRef.current.forEach((bomb) => {
      ...
      const greenComponent = Math.max(
        0,
        Math.floor(255 * (timeRemaining / BOMB_LIFETIME)),
      );
    ...
    if (!gameStarted && !gameOver) {
      ...
      if (!WRAP_AROUND) {
        context.fillText(
          'Avoid hitting walls and your trail!',
          CANVAS_WIDTH / 2,
          CANVAS_HEIGHT / 2 + 60,
        );
      } else {
        context.fillText(
          'You can go through walls but avoid hitting your trail!',
          CANVAS_WIDTH / 2,
          CANVAS_HEIGHT / 2 + 60,
        );
      }
